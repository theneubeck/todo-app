...
- `test/view/designAndStructure.spec.ts` — two authorized edit regions, no others:
  - **Lines 93, 96 (brand assertion)**: update the `it("renders the TaskStream brand …")` test description to `it("renders the TODO brand in the top app bar")` and update its expectation from `"TaskStream"` to `"TODO"`.
  - **Lines 132–138 (PEOPLE-header assertion)**: invert the test from asserting presence to asserting absence under the new auto-hide rule. Update the test description to `it("hides the PEOPLE section header when no @-tags exist in the sidebar")` and update its body so it asserts that the collected `[data-section-header]` values do **not** include `'PEOPLE'` (e.g., `expect(headers).to.not.include('PEOPLE')`). The design-and-structure fixture set in `buildTasks()` has zero `@`-tagged tasks, so under AC#8 the PEOPLE section is correctly absent. The PROJECTS assertion at lines 124–130 stays unchanged because the fixtures still contain `#`-tag-equivalents (`work`, `q2`, `personal`, `reading`) that the renderer surfaces as PROJECTS entries.
  - **Do not edit any other line of this file.** In particular, `buildTasks()` (lines 20–61), the PROJECTS-header assertion (lines 124–130), the remaining-count assertion (line 151), the row-count assertion (lines 180–181), and every other test must remain byte-for-byte unchanged. The fixture stays at three tasks — no row-count or remaining-count change is required or permitted.
