...
Wire all three into a single command for CI:
...
async function verify() {
  ...
  // Screenshot 2 — after interaction (e.g. toggle)
...
```json
"scripts": {
  "verify": "ts-node test/verify/<pattern>.verify.ts"
}
```
...
// expect(before).to.include('- [ ]')   // pre-condition

// ... click, wait ...

const after = fs.readFileSync(fixturePath, 'utf-8')
// expect(after).to.include('- [x]')    // post-condition

// Always restore
fs.writeFileSync(fixturePath, before, 'utf-8')
```
...
Write a findings block in `TODO-POC.md` under the verified pattern, using this template:
