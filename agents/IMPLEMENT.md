...
describe('Reminders pattern', () => {
  ...
  before(async () => {
    // Load the pattern HTML — set window.todoz before the script runs
  ...
  it('renders a sidebar entry for each unique tag', async () => {
    ...
    expect(items.length).to.equal(1) // one unique tag in fixtures
  })

  // ... more tests from the plan's BDD test list
})
```
...
- Fixture data in the test file must match `AGENTS.md` schema exactly (same fields, same types)
...
describe('parseTodo', () => {
  ...
  it('parses top-level checkboxes from the body', () => {
    const raw = fs.readFileSync(
      'test/fixtures/vault/todos/my-task-2026-05-04.md',
      'utf-8'
    )
    const task = parseTodo(raw)
    expect(task.subtasks[0].text).to.equal('Top level task')
    expect(task.subtasks[0].done).to.be.false
  })
})
```
...
Add `data-*` attributes to every meaningful DOM element so Tallahassee and Playwright can query them:
...
The test command is defined in `package.json`. If it is not yet defined, add:
...
- [ ] A final screenshot has been taken (use Playwright headlessly or the Electron app directly)
