...
After Playwright writes the PNGs to `tmp/`, open each one with the `Read` tool — it returns the image to you as a multimodal observation.
...
Then proceed to **Step 7 — Commit on green**.

If a check fails because of Implement's code:

> **Verify failed. Returning to Implement.**

Then provide the exact failure: the test name and error, or the acceptance criterion the screenshot did not satisfy with your one-sentence reason. Do not suggest a fix — that is Implement's job. **Do not commit.**

If the failure is a plan problem (criterion is unverifiable, scenario contradicts itself):

> **Plan problem detected. Returning to Plan agent.**

See `features/<slug>/notes.md` → Problems. **Do not commit.**

---

## Step 7 — Commit on green

Only run this step after declaring **"Verify complete. Pattern <slug> is done."** Skip it on any failure verdict.

### Stage feature-owned paths explicitly

Use `git add` with explicit paths — never `git add -A` or `git add .`. Untracked files outside the feature's scope (personal scratchpads like `fixes.md`, `todo.md`, `scratch*`) must NOT be staged.

Owned paths to stage:

- `features/<slug>/` (whole directory — plan.md, notes.md, any reference assets)
- `test/features/<slug>.feature`
- `test/step_defs/<slug>.steps.ts`
- `test/patterns/<slug>.spec.ts` (if it exists)
- Any `test/data/*.spec.ts` newly added by Implement for this feature — confirm via `git status` that they are untracked, then stage only those
- Any `test/fixtures/...` paths newly added by Implement for this feature — same rule
- `test/verify/<slug>.verify.ts`
- `package.json` if Implement modified it

**Do NOT stage `tmp/*` screenshots.** The `tmp/` directory is gitignored (only `tmp/.gitkeep` is tracked). Screenshots are evidence for the current verify run only — they regenerate every time the verify chain runs, so committing them is pure churn.
- All modified tracked files under `src/` (`git diff --name-only -- src/`) — these are Implement's source changes

Run `git status` first; surface anything untracked outside the scope and confirm with the user before staging if you are unsure.

### Commit message

Match the existing repo style — short imperative subject, optional body. Run `git log --oneline -10` to confirm the convention.

```
Implement <slug> feature

<2–3 line summary distilled from the plan's Pattern summary>
...
- Never `--no-verify` — if a pre-commit hook fails, surface the error and stop. Do not retry with `--no-verify`.
- Never push. Commits stay local; the user pushes manually.
- If the working tree has unrelated uncommitted changes you cannot attribute to this feature (e.g., untracked files in unfamiliar locations), stop and ask the user before committing — do not silently include or exclude them.

### After commit

Run `git status` to confirm the commit landed and report the resulting commit hash and the files included to the user.
